package br.edu.ufape.controller;

import java.util.List;
import java.util.Optional;

import javax.validation.Valid;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.validation.BindingResult;
import org.springframework.validation.annotation.Validated;
import org.springframework.web.bind.annotation.ModelAttribute;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;

import br.edu.ufape.model.Formacao;
import br.edu.ufape.repository.FormacaoDAOI;

@Controller
// Definindo a "url" quando estamos nos referindo a Formação.
@RequestMapping ("/formacao")
public class FormacaoController {
	// Nota geral:
	// Foi definido no properties qual o caminho a ser procurado quando estamos nos referindo as views.
	// "{}" Se faz para adaptar ao id do que foi selecionado.

	// Para a persistencia de dados.
	@Autowired
	FormacaoDAOI formacaoDAOI;


	// Encaminhamento e captação no banco da pagina apresentada inicialmente.
	// OBS:RequestMappin com dois "caminhos", podendo ser utilizado /all e / para o acesso.
	@RequestMapping(value= {"/all", "/", ""})
	public String findAllFormacao(Model model) {

		// Retorna uma lista de todos os obejtos de "Formação".
		List<Formacao> formacoes = formacaoDAOI.findAll();
		// Passando para o modelo a colection/lista.
		model.addAttribute("formacoes", formacoes);
		// Encaminha o fluxo para a view.
		return "Formacao/listaformacao";	
	}


	// Atualização de uma "formação" escolhida e encaminhamento para a pagina de update.
	@RequestMapping(value= "/update/{id}" )
	public String update(@PathVariable("id") Long id, Model model) {

		// Procura o objeto Formacao pelo seu id.
		Optional<Formacao> formacao = formacaoDAOI.findById(id);
		// Passando para o modelo o objeto captado acima.
		model.addAttribute("formacaoForm", formacao.get());
		// Verifica se o obejto está vazio.
		if (!formacao.isEmpty()) {
			// Encaminha o fluxo para a view.
			return "Formacao/updateformacao";
		} else {
			// Caso ocorra um "erro" na verificação, será mostrada uma mensagem logo acima da tabela apresentada na view.
			model.addAttribute("errors","Formacao não encontrada!");
			// Encaminha o fluxo para a view.
			return "Formacao/listaformacao";
		}
	}


	// Validação e ação de mudança no bando. Encaminhamento para a pagina de listagem das formações.
	@RequestMapping(value= "/update2" , method = RequestMethod.POST)
	public String update2(@Valid @ModelAttribute("formacaoForm") Formacao form,
			BindingResult bindingResult,
			Model model
			) {

		//Verificação de erro
		if(bindingResult.hasErrors( )) {
			return "Formacao/updateformacao";
		}

		// Salvando as alterações realizadas através do form.
		formacaoDAOI.save(form);
		// Mensagem que irá ser apresentada logo acima da tabela apresentada.
		model.addAttribute("msgs", "Formacao alterada com sucesso!");
		// Encaminha o fluxo para a url.
		return "redirect:/formacao/all";
	}


	// Deleta o elemento selecionado.
	@RequestMapping(value= "/delete/{id}" , method = RequestMethod.GET)
	public String delete(@PathVariable("id") Long id, Model model) {

		// Procura o objeto Formacao pelo seu id.
		Optional<Formacao> formacao = formacaoDAOI.findById(id);
		// Verifica se o obejto está vazio.
		if (!formacao.isEmpty()) {
			// Deleta o objeto captado anteriormente.
			formacaoDAOI.delete(formacao.get());
			// Mensagem que irá ser apresentada logo acima da tabela apresentada.
			model.addAttribute("msgs", "Formacao deletada com sucesso!");
		} else {
			// Caso ocorra um "erro" na verificação, será mostrada uma mensagem logo acima da tabela apresentada na view.
			model.addAttribute("errors","Formacao não encontrada!");		
		}
		//Encaminha o fluxo para a url.
		return "redirect:/formacao/all";	
	}


	// Cria uma nova Formação.
	@RequestMapping(value= "/new")
	public String novo(Long id, Model model) {

		// Cria e passa para o model uma "Formacao" vazia.
		model.addAttribute("formacaoForm", new Formacao());	
		// Encaminha o fluxo para a view, que irá possibilitar o preenchimento dos dados.
		return "Formacao/newformacao";

	}


	// Conclui as mudanças realizadas na view de criação.
	@RequestMapping(value= "/new2")
	public String novo2 (@Valid @ModelAttribute("formacaoForm") Formacao form,
			BindingResult bindingResult,
			Model model
			) {
		//Verificação de erro
		if(bindingResult.hasErrors( )) {
			return "Formacao/newformacao";
		}
		// Salva as alterações realizadas através do form.
		formacaoDAOI.save(form);
		// Mensagem que irá ser apresentada logo acima da tabela apresentada.
		model.addAttribute("msgs", "Formacao cadastrada com sucesso");
		// Encaminha o fluxo para a url.
		return "redirect:/formacao/all";

	} 

}